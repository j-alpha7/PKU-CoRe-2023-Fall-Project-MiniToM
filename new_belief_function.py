    ...
    for i in range(11):
        for j in range(11):
            ...
            if j+1 < 11 and Animal_in_view(i,j+1,human_pos,obs_radius) == 0:
                cnt+=1
            belief[i][j]=belief[i][j]/direct_num*cnt
            sum+=belief[i][j]
    belief=np.divide(belief,sum)
    belief=Generate(belief)
    sum = 1.0
    for i in range(max(human_pos[0]-obs_radius,0),min(human_pos[0]+obs_radius,11-1)+1):
        for j in range(max(human_pos[1]-obs_radius,0),min(human_pos[1]+obs_radius,11-1)+1):
            sum-=belief[i][j]
            belief[i][j]=0
    return belief/sum
...
def Reward_calc(gamma,k,c,threshold,belief,x,y): #计算E[γ^(k*dis+c)]
    belief_tensor = torch.tensor(belief)
    i_tensor = torch.arange(0, 11).reshape(11, 1)
    j_tensor = torch.arange(0, 11).reshape(1, 11)
    abs_diff_x = torch.abs(x - i_tensor) 
    abs_diff_y = torch.abs(y - j_tensor)
    gamma_power = gamma ** (k[0] * abs_diff_x + k[1]*abs_diff_y + 
                            k[2] * torch.nn.functional.relu(abs_diff_x - threshold)+ 
                            k[3] * torch.nn.functional.relu(abs_diff_y - threshold) + c)
    weighted_sum = torch.sum(belief_tensor * gamma_power)
    return weighted_sum

def Dist_calc(belief, k, c, threshold, x, y):
    belief_tensor = torch.tensor(belief)
    i_tensor = torch.arange(0, 11).reshape(11, 1)
    j_tensor = torch.arange(0, 11).reshape(1, 11)
    abs_diff_x = torch.abs(x - i_tensor) 
    abs_diff_y = torch.abs(y - j_tensor)
    estimate_time = (k[0] * abs_diff_x + k[1]*abs_diff_y + 
                        k[2] * torch.nn.functional.relu(abs_diff_x - threshold)+ 
                        k[3] * torch.nn.functional.relu(abs_diff_y - threshold) + c)
    weighted_sum = torch.sum(belief_tensor * estimate_time)
    return weighted_sum
...
def Belief_pred(belief_rabbit,belief_sheep,human_pos):#给定当下的belief和hunter位置，预测接下来走各个方向的收益,gamma表示折扣因子，k*dis+c是和期望步数相关的一次函数
    ...
    def score_func(gamma, k, c, threshold, ratio_reward):
        ans=torch.full((5,), -torch.inf, dtype=torch.float32)
        ans[0] = torch.max(Reward_calc(gamma,k,c,threshold,belief_rabbit,i,j), Reward_calc(gamma,k,c,threshold,belief_sheep,i,j)*ratio_reward)
        if i > 0:
            ans[1] = torch.max(Reward_calc(gamma,k,c,threshold,belief_rabbit,i-1,j), Reward_calc(gamma,k,c,threshold,belief_sheep,i-1,j)*ratio_reward)
        if i+1 < 11:
            ans[2] = torch.max(Reward_calc(gamma,k,c,threshold,belief_rabbit,i+1,j), Reward_calc(gamma,k,c,threshold,belief_sheep,i+1,j)*ratio_reward)
        if j > 0:
            ans[3] = torch.max(Reward_calc(gamma,k,c,threshold,belief_rabbit,i,j-1), Reward_calc(gamma,k,c,threshold,belief_sheep,i,j-1)*ratio_reward)
        if j+1 < 11:
            ans[4] = torch.max(Reward_calc(gamma,k,c,threshold,belief_rabbit,i,j+1), Reward_calc(gamma,k,c,threshold,belief_sheep,i,j+1)*ratio_reward)
